var YZM = {
		...
		$.ajax({
			...
			success: function(e) {
				...
				if (config.group < config.group_x && YZM.ads.state == 'on' && config.group != '') {
					...
					YZM.play(window.atob(config.url));
				}
			}
		});
	},
	'play': function(url) {
		...
		$(function() {
			$(".alphaYplayer-setting-speeds,.alphaYplayer-setting-speed-item").on("click", function() {
				$(".speed-stting").toggleClass("speed-stting-open");
			});
			$(".speed-stting .alphaYplayer-setting-speed-item").click(function() {
				$(".alphaYplayer-setting-speeds  .title").text($(this).text());
			});
		});
		$(".alphaYplayer-fulloff-icon").on("click", function() {
			YZM.dp.fullScreen.cancel();
		});
		$(".alphaYplayer-showing").on("click", function() {
			YZM.dp.play();
			$(".vod-pic").remove();
		});
		if (config.title != '') {
	...
	'def': function() {
		...
		YZM.playtime = Number(YZM.getCookie("time_" + window.atob(config.url)));
	...
	'video': {
		...
		'con_play': function() {
			...
			$(".alphaYplayer-cplayer").append(cplayer);
	...
	'jump': {
		'def': function() {
			h = ".alphaYplayer-setting-jfrist label";
			l = ".alphaYplayer-setting-jlast label";
			f = "#fristtime";
			j = "#jumptime";
				...
				if (YZM.lasts == 1) {
					setInterval(function() {
						...
						if (YZM.lastt > 0 && e < YZM.lastt) {
							YZM.setCookie("time_" + window.atob(config.url), "", -1);
							YZM.video.next();
						};
					}, 1000);
				};
	...
	'danmu': {
		'send': function() {
			g = $(".yzm-alphaYplayer-send-icon");
			d = $("#dmtext");
			h = ".alphaYplayer-comment-setting-";
		...
		'list': function() {
			$(".alphaYplayer-list-icon,.yzm-alphaYplayer-send-icon").on("click", function() {
				$(".list-show").empty();
				$.ajax({
			...
			$("div.alphaYplayer-comment-box:last").append(liyih);
			$(".alphaYplayer-watching-number").text(up.usernum);
			$(".alphaYplayer-info-panel-item-title-amount .alphaYplayer-info-panel-item-title").html("违规词");
			for (var i = 0; i < up.pbgjz.length; i++) {
				var gjz_html = "<e>" + up.pbgjz[i] + "</e>";
				$("#vod-title").append(gjz_html);
			}
			add('.alphaYplayer-list-icon', ".alphaYplayer-danmu", 'show');
	...
	'timeupdateHandler': function() {
		YZM.setCookie("time_" + window.atob(config.url), YZM.dp.video.currentTime, 24);
	},
	...
	'player': {
		...
		'adplay': function(url) {
			...
			$('.alphaYplayer-controller,.alphaYplayer-cplayer,.alphaYplayer-logo,#loading-box,.alphaYplayer-controller-mask').remove();
			$('.alphaYplayer-mask').show();
